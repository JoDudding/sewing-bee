...
ratings |>
  ...
  filter(sum(!is.na(total_viewers_m)) >= 8) |>
  ungroup() |>
  ggplot(aes(episode, total_viewers_m,
...
ratings |>
  filter(episode_type != "Middle") |>
  select(episode_type, series, total_viewers_m) |>
  ggplot(aes(series, total_viewers_m,
    colour = episode_type, fill = episode_type,
    group = episode_type
  )) +
  geom_line() +
  geom_point() +
...
ratings |>
  filter(episode_type != "Middle") |>
  ggplot(aes(
    y = fct_rev(series), x = total_viewers_m, group = series,
    colour = episode_type
  )) +
  geom_line() +
  geom_point() +
...
ratings |>
  filter(episode_type != "Middle") |>
  ggplot(aes(
    x = fct_reorder(episode_type, episode), y = total_viewers_m,
...
ratings_first_last |>
  ...
  geom_col() +
  geom_hline(
    yintercept = median(ratings_first_last$pct_change),
